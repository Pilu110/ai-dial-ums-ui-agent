...
class ConversationManager:
    ...
    async def create_conversation(self, title: str) -> dict:
        ...
        logger.info(f"Created conversation {conversation_id}", extra={"title": title})
        return conversation
    ...
    async def list_conversations(self) -> list[dict]:
        ...
        for conversation_id in conversation_ids:
            conversation_data = await self.redis.get(f"{CONVERSATION_PREFIX}{conversation_id}")
            if conversation_data:
                conv = json.loads(conversation_data)
    ...
    async def chat(
        ...
        """
        logger.info(f"Processing chat request", extra={"conversation_id": conversation_id, "stream": stream})
